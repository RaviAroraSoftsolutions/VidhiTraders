  ...
  return async (dispatch) => {
    ...
    try {
      const response = await api.get(
        `/api/admin/orders?pageNumber=${reqData?.pageNumber}&pageSize=${reqData?.pageSize}&orderStatus=${reqData?.orderStatus}&searchTerm=${reqData?.searchTerm}`,{signal:reqData?.signal}
      );
